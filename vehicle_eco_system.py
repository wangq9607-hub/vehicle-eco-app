...
with tab1:
    ...
    st.subheader("📋 产品数据总表")
    st.caption("💡 提示：此表格仅供查看。如需修改数据，请在下方的【⚙️ 数据操作区】进行。")
    ...
    if not df_p.empty:
        ...
        with col_edit:
            st.info("✏️ **编辑产品信息**")
            edit_p_name = st.selectbox("1. 选择要修改的产品", df_p['产品名称'].tolist(), key="edit_p_select")
            if edit_p_name:
...
with tab2:
    ...
    if not df_i.empty:
        ...
        with col_edit_i:
            st.info("✏️ **编辑接口信息**")
            edit_i_name = st.selectbox("1. 选择要修改的接口", df_i['接口名称'].tolist(), key="edit_i_select")
            if edit_i_name:
